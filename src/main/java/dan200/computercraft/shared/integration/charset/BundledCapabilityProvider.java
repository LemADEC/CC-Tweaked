...
final class BundledCapabilityProvider implements ICapabilityProvider
{
    ...
    @Override
    public boolean hasCapability( @Nonnull Capability<?> capability, @Nullable Facing side )
    {
    ...
    @Nullable
    @Override
    public <T> T getCapability( @Nonnull Capability<T> capability, @Nullable Facing side )
    {
            ...
            if( emitter == null )
            {
                if( side == null )
                {
                    emitter = emitters[index] = () -> {
                        int flags = 0;
                        for( Facing facing : Facing.VALUES ) flags |= tile.getBundledRedstoneOutput( facing );
